...
class Calculator():
    def __init__(self, initial, final, cap, charger, dates, times, postcodes, location):
        ...
        self.location_id = ""
        self.json_content = self.get_json_content(self.get_weather_api(dates))
    ...
    def cost_calculation(self):
        ...
        if self.is_surcharged(date_time):
            print("cost_calculation -> is_surcharged")
            surcharge_factor = 1.1
            if self.is_surcharged_to_not():
                print("cost_calculation -> is_surcharged_to_not")
                cost = self.cost_par(int(self.initial_state), int(self.final_charge), 0.5, surcharge_factor)
                return cost

        else:
            surcharge_factor = 1
            print("cost_calculation -> is_not_surcharged")
            if self.is_not_to_surcharged():
                print("cost_calculation -> is_not_to_surcharged")
                cost = self.cost_par(int(self.initial_state), int(self.final_charge), 0.5, surcharge_factor)
                return cost

        if self.is_peak(self.start_time + ":00"):
            print("cost_calculation -> is_peak")
            if self.is_peak_to_off():
                print("cost_calculation -> is_peak_to_off")
                ...
                print("cost_calculation -> NOT is_peak_to_off")
                cost = self.cost_par(int(self.initial_state), int(self.final_charge), 1, surcharge_factor)

        else:
            print("cost_calculation -> NOT is_peak")
            if self.is_off_to_peak():
                print("cost_calculation -> is_off_to_peak")
                mid = self.middle_charge()
                ...
                print("cost_calculation -> NOT is_off_to_peak")
                cost = self.cost_par(int(self.initial_state), int(self.final_charge), 0.5, surcharge_factor)
    ...
    def time_diff(self, start_time):
        ...
        elif not time_format(start_time):
            raise ValueError("Time must be in HH:MM format with numerical values only")
        else:
            split_time = str(start_time).split(":")
            if int(split_time[0]) > 24 or int(split_time[0]) < 0:
                raise ValueError("Invalid time")
            elif int(split_time[1]) > 60 or int(split_time[1]) < 0:
                raise ValueError("Invalid time")
            elif not str(split_time[0]).isnumeric() or not str(split_time[1]).isnumeric():
                raise ValueError("Time must be of numerical values only")
    ...
    def is_peak(self, start_time):
        ...
        """
        if not time_sec_format(start_time):
            raise ValueError("Time must be in HH:MM:SS format with numerical values only")
        else:
            split_time = str(start_time).split(":")
            if int(split_time[0]) > 24 or int(split_time[0]) < 0:
                raise ValueError("Invalid time")
                ...
                raise ValueError("Invalid time")
            elif int(split_time[2]) > 60 or int(split_time[2]) < 0:
                raise ValueError("Invalid time")
            elif not str(split_time[0]).isnumeric() or not str(split_time[1]).isnumeric() or not str(split_time[2]).isnumeric():
                raise ValueError("Time must be of numerical values only")
    ...
    def get_daylight_length_charge(self):
        ...
        if (start_time_dt < sun_rise) and (sun_rise <= end_time <= sun_set):
            print("get_daylight_length_charge -> start before sunrise and ends before sunset")
            print("night > day ")
            return sun_rise, end_time
        # .....SR...st..et.....SS......
        elif (sun_rise <= start_time_dt <= sun_set) and (sun_rise <= end_time <= sun_set):
            print("get_daylight_length_charge -> start after sunrise and ends before sunset")
            print("day")
            return start_time_dt, end_time
        # .....SR......st......SS..et..
        elif (sun_rise <= start_time_dt <= sun_set) and (end_time > sun_set):
            print("get_daylight_length_charge -> start after sunrise and ends after sunset")
            print("day > night ")
            return start_time_dt, sun_set
        # .st..SR..............SS..et..
        elif (start_time_dt < sun_rise) and (end_time > sun_set):
            print("get_daylight_length_charge -> start before sunrise and ends after sunset")
            print("night > day > night ")
            return sun_rise, sun_set
        # .....SR..............SS.st.et..
        elif (start_time_dt > sun_set) and (end_time > sun_set):
            print("get_daylight_length_charge -> start after sunset and ends after sunset")
            print("night but after sunset")
            return None
        # ..st.et...SR..............SS...
        elif (start_time_dt < sun_rise) and (end_time < sun_rise):
            print("get_daylight_length_charge -> start before sunrise and ends before sunrise")
            print("night but before sunrise ")
            return None
        else:
            print("get_daylight_length_charge -> else")
            return None
    ...
    def check_future_date(self, dates):
        ...
        :param dates: takes in date as a datetime format
        :return: a boolean
        """
        self.test_valid_dates(dates)
    ...
    def get_previous_two_dates(self, dates):
        ...
        if type(dates) != datetime:
            raise ValueError("Input should be in datetime format")
        elif not datetime_format(str(dates)):
            raise ValueError("Date must be in YYYY-MM-DD HH:MM:SS format with numerical values only")
        else:
            split_dates = str(dates).split(" ")
            split_date = str(split_dates[0]).split("-")
            if calendar.isleap(int(split_date[0])):
                if split_date[1] == "02":
                    if int(split_date[2]) < 1 or int(split_date[2]) > 29:
                        raise ValueError("Invalid date")
            else:
                if split_date[1] == "02":
                    if int(split_date[2]) < 1 or int(split_date[2]) > 28:
                        raise ValueError("Invalid date")
            if split_date[1] == "01" or "03" or "05" or "07" or "08" or "10" or "12":
                if int(split_date[2]) < 1 or int(split_date[2]) > 31:
                    raise ValueError("Invalid Date")
            elif int(split_date[1]) < 0 or int(split_date[1]) > 12:
                raise ValueError("Invalid Date")
            else:
                if int(split_date[2]) < 1 or int(split_date[2]) > 30:
                    raise ValueError("Invalid Date")
                if int(split_date[0]) < 2008:
                    raise ValueError("Invalid Date: Dates only start from 01/07/2008")
                elif int(split_date[0]) == 2008:
    ...
    def cal_is_peak(self, net, surcharged, times):
        ...
        """
        if type(net) == str:
            raise ValueError("Net should be a numerical type only")
        elif type(surcharged) == str:
            raise ValueError("Surcharged should be a numerical type only")
        elif type(times) != str:
            raise ValueError("Times should be of String format")
        if not time_sec_format(times):
            raise ValueError("Time must be in HH:MM:SS format with numerical values only")

        elif surcharged != 1 and surcharged != 1.1:
            raise ValueError("Surcharge can only be 1 or 1.1 in numerical form.")
        else:
    ...
    def cal_solar_energy_cost(self):
        ...
        if self.get_daylight_length_charge() is None:
            print("cal_solar_energy_cost -> get_daylight_length_charge is None")
            return 0
        ...
        if self.is_surcharged(start_time.strftime('%d/%m/%Y %H:%M:%S')):
            print("cal_solar_energy_cost -> is_surcharged == True")
            surcharged_val = 1.1

        # When difference of length is 0 where start time is 15:15 and end time is 15:45
        if length == 0:
            print("cal_solar_energy_cost -> length == 0 (less than an hour)")
        ...
        # When difference of length is more than 1 where start time is 15:15 and end time is 16:45
            ...
            print("cal_solar_energy_cost -> length >= 1 (at least than an hour)")
            ...
            if length >= 2:
                print("cal_solar_energy_cost -> length >= 2 (more than an hour)")
    ...
    def final_cost_calculation(self):
        ...
        if self.check_future_date(self.start_date):
            print("final_cost_calculation -> check_future_date == True")
            ...
            print("final_cost_calculation -> check_future_date == False")
            solar_cost = self.cal_solar_energy_cost()

        final_cost = cost_charging - solar_cost
        if final_cost < 0:
            print("final_cost_calculation is negative")
            final_cost = 0
    ...
    def test_valid_start_date(self):
        if self.start_date == "":
            ...
            raise ValueError("Input should be in String format")
        elif not date_format(self.start_date):
            raise ValueError("Date must be in DD/MM/YYYY format with numerical values only")
        else:
            split_date = str(self.start_date).split("/")
            if calendar.isleap(int(split_date[2])):
                if split_date[1] == "02":
                    if int(split_date[0]) < 1 or int(split_date[0]) > 29:
                        raise ValueError("Invalid date")
            else:
                if split_date[1] == "02":
                    if int(split_date[0]) < 1 or int(split_date[0]) > 28:
                        raise ValueError("Invalid date")
            if split_date[1] == "01" or "03" or "05" or "07" or "08" or "10" or "12":
                if int(split_date[0]) < 1 or int(split_date[0]) > 31:
                    raise ValueError("Invalid Date")
            elif int(split_date[1]) < 0 or int(split_date[1]) > 12:
                raise ValueError("Invalid Date")
            else:
                if int(split_date[0]) < 1 or int(split_date[0]) > 30:
                    raise ValueError("Invalid Date")
                if int(split_date[2]) < 2008:
                    raise ValueError("Invalid Date: Dates only start from 01/07/2008")
                elif int(split_date[2]) == 2008:
                    if int(split_date[1]) < 7:
                        raise ValueError("Invalid Date: Dates only start from 01/07/2008")
    ...
    def test_valid_start_time(self):
        if self.start_time is None:
            ...
            raise ValueError("Input should be in String format")
        elif not time_format(self.start_time):
            raise ValueError("Time must be in HH:MM format with numerical values only")
        else:
            split_time = str(self.start_time).split(":")
            if int(split_time[0]) > 24 or int(split_time[0]) < 0:
                raise ValueError("Invalid time")
            elif int(split_time[1]) > 60 or int(split_time[1]) < 0:
                raise ValueError("Invalid time")
    ...
    def test_valid_surchaged(self, dates):
        if type(dates) != str:
            raise ValueError("Input should be in String format")
        elif not date_time_format(dates):
            raise ValueError("Date must be in DD/MM/YYYY HH:MM:SS format with numerical values only")
        else:
            ...
            split_time = str(split_dates[1]).split(":")
            if int(split_time[0]) > 24 or int(split_time[0]) < 0:
                raise ValueError("Invalid time")
            elif int(split_time[1]) > 60 or int(split_time[1]) < 0:
                raise ValueError("Invalid time")
                ...
                raise ValueError("Invalid time")
            elif not str(split_time[0]).isnumeric() or not str(split_time[1]).isnumeric() or not str(
                    split_time[2]).isnumeric():
                raise ValueError("Time must be of numerical values only")

    def test_valid_dates(self, dates):
        if type(dates) != str:
            raise ValueError("Input should be in String format")
        elif not date_format(dates):
            raise ValueError("Date must be in DD/MM/YYYY format with numerical values only")
        else:
            split_date = str(dates).split("/")
            if calendar.isleap(int(split_date[2])):
                if split_date[1] == "02":
                    if int(split_date[0]) < 1 or int(split_date[0]) > 29:
                        raise ValueError("Invalid date")
...
# helper function to enforce date format
def date_format(instr):
        match = re.match("[0-9][0-9]/[0-9][0-9]/[0-9][0-9][0-9][0-9]", instr)
        return bool(match)


# helper function to enforce time format
def time_format(instr):
    match = re.match("[0-9][0-9]:[0-9][0-9]", instr)
    return bool(match)


def time_sec_format(instr):
    match = re.match("[0-9][0-9]:[0-9][0-9]:[0-9][0-9]", instr)
    return bool(match)


def date_time_format(instr):
    match = re.match("[0-9][0-9]/[0-9][0-9]/[0-9][0-9][0-9][0-9] [0-9][0-9]:[0-9][0-9]:[0-9][0-9]", instr)
    return bool(match)


def datetime_format(instr):
    match = re.match("[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9] [0-9][0-9]:[0-9][0-9]:[0-9][0-9]", instr)
    return bool(match)
...
if __name__ == "__main__":
    calc = Calculator("10", "20", "31", "3", "25/08/2021", "14:00", "7250", "LAUNCESTON")
    calc.final_cost_calculation()